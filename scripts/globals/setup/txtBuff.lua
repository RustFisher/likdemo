...
buff.conf("fetter", "定身", "ability/NatureGroundingTotem")
...
buff.conf("shieldRegen", nil, "ability/HolyBlessingOfProtection")
buff.conf("punish", nil, "ability/ValkyrieArmed")
buff.conf("punishCur", nil, "ability/ValkyrieArmed")
buff.conf("punishRegen", nil, "ability/GodofWar")
buff.conf("weight", nil, "other/DustOfAppearance")
buff.conf("weightCur", nil, "other/DustOfAppearance")
...
for _, k in ipairs(ENCHANT_TYPES) do
    ...
    buff.conf(SYMBOL_E .. k, name .. "<强化>", ei[k])
end
for _, v in ipairs(ATTR_ODDS) do
    local k = string.replace(v, SYMBOL_E, '')
    local lb, ic = buff.conf(k)
    buff.conf(SYMBOL_ODD .. v, lb .. '<几率>', ic)
end
for _, v in ipairs(ATTR_RESISTANCE) do
    local k = string.replace(v, SYMBOL_E, '')
    local lb, ic = buff.conf(k)
    buff.conf(SYMBOL_RES .. v, lb .. '<抗性>', ic)
end